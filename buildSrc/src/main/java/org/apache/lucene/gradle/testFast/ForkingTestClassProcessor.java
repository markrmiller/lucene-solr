...
public class ForkingTestClassProcessor implements TestClassProcessor {
    ...
    RemoteTestClassProcessor forkProcess() {
        ...
        //File cwd = new File(builder.getJavaCommand().getWorkingDir(), "jvm-" + count.getAndIncrement());
        //cwd.mkdirs();
        //File tmpDir = new File(Paths.get(System.getProperty("java.io.tmpdir")).toFile(), "jvm-" + count.getAndIncrement());
        //tmpDir.mkdirs();
        //builder.getJavaCommand().setWorkingDir(cwd);
        //builder.getJavaCommand().jvmArgs("-Dorg.gradle.native=false -Djava.io.tmpdir=" + tmpDir.getAbsolutePath() + " -DtempDir=" + tmpDir.getAbsolutePath());
