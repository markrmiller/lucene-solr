...
public class SplitShardCmd implements OverseerCollectionMessageHandler.Cmd {
  ...
  public boolean split(ClusterState clusterState, ZkNodeProps message, NamedList<Object> results) throws Exception {
    ...
    try {
      parentShardLeader = zkStateReader.getLeaderRetry(collectionName, slice.get());
    ...
    try {
        ...
        if (oSlice != null) {
          ...
          if (state == Slice.State.ACTIVE) {
            ...
            try {
              ...
              throw new SW.Exp("Unable to delete already existing sub shard: " + subSlice, e);
            }
      ...
      throw new SW.Exp("Error executing split operation for collection: " + collectionName + " parent shard: " + slice, e);
    } finally {
  ...
  private void cleanupAfterFailure(ZkStateReader zkStateReader, String collectionName, String parentShard,
    ...
    if (sendUpdateState) {
      try {
        ...
        throw new SW.Exp("Cleanup failed after failed split of " + collectionName + "/" + parentShard + ": (slice state changes)", e);
      }
    }
    ...
    for (String subSlice : subSlices) {
      ...
      try {
        ...
        throw new SW.Exp("Cleanup failed after failed split of " + collectionName + "/" + parentShard + ": (deleting existing sub shard " + subSlice + ")", e);
      }
    }
  }
  ...
  public static String fillRanges(SolrCloudManager cloudManager, ZkNodeProps message, DocCollection collection, Slice parentSlice,
    ...
    try {
      ...
      throw new SW.Exp("Invalid numeric value of 'fuzz': " + fuzzStr, e);
    }
      ...
      if (ranges.length == 0 || ranges.length == 1) {
        ...
        for (int i = 0; i < ranges.length; i++) {
          ...
          try {
            ...
            throw new SW.Exp("Exception in parsing hexadecimal hash range: " + r, e);
          }
  ...
  public static boolean lockForSplit(SolrCloudManager cloudManager, String collection, String shard) throws Exception {
    ...
    synchronized (stateManager) {
      ...
      try {
        ...
        throw new SW.Exp("Can't lock parent slice for splitting (another split operation running?): " +
            collection + "/" + shard, e);
      }
      return true;
    }
